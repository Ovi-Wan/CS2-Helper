...
import anubisCTLineups from "../../../data/anubis-ct-lineups";
...
export default function CtAnubis() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: anubisCTLineups.length,
      smoke: anubisCTLineups.filter(item => item.type === "smoke").length,
      molly: anubisCTLineups.filter(item => item.type === "molly").length,
      flash: anubisCTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return anubisCTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Anubis – CT Utility</h1>
