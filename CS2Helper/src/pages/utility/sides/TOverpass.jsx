...
import overpassTLineups from "../../../data/overpass-t-lineups";
...
export default function TOverpass() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: overpassTLineups.length,
      smoke: overpassTLineups.filter(item => item.type === "smoke").length,
      molly: overpassTLineups.filter(item => item.type === "molly").length,
      flash: overpassTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return overpassTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Overpass – T Utility</h1>
