...
import dust2CTLineups from "../../../data/dust2-ct-lineups";
...
export default function CtDust2() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: dust2CTLineups.length,
      smoke: dust2CTLineups.filter(item => item.type === "smoke").length,
      molly: dust2CTLineups.filter(item => item.type === "molly").length,
      flash: dust2CTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return dust2CTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Dust II – CT Utility</h1>
