...
export default function CtOverpass() {
  ...
  const counts = useMemo(() => {
    return {
      all: overpassCTLineups.length,
      smoke: overpassCTLineups.filter(item => item.type === "smoke").length,
      molly: overpassCTLineups.filter(item => item.type === "molly").length,
      flash: overpassCTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return overpassCTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Overpass – CT Utility</h1>
