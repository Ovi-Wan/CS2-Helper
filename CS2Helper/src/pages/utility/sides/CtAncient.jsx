...
import ancientCTLineups from "../../../data/ancient-ct-lineups";
...
export default function CtAncient() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: ancientCTLineups.length,
      smoke: ancientCTLineups.filter(item => item.type === "smoke").length,
      molly: ancientCTLineups.filter(item => item.type === "molly").length,
      flash: ancientCTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return ancientCTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Ancient – CT Utility</h1>
