...
import dust2TLineups from "../../../data/dust2-t-lineups";
...
export default function TDust2() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: dust2TLineups.length,
      smoke: dust2TLineups.filter(item => item.type === "smoke").length,
      molly: dust2TLineups.filter(item => item.type === "molly").length,
      flash: dust2TLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return dust2TLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Dust II – T Utility</h1>
