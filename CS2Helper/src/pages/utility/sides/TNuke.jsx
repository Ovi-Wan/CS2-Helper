...
import nukeTLineups from "../../../data/nuke-t-lineups";
...
export default function TNuke() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: nukeTLineups.length,
      smoke: nukeTLineups.filter(item => item.type === "smoke").length,
      molly: nukeTLineups.filter(item => item.type === "molly").length,
      flash: nukeTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return nukeTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Nuke – T Utility</h1>
