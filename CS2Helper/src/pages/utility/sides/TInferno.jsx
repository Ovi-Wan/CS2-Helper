...
import infernoTLineups from "../../../data/inferno-t-lineups";
...
export default function TInferno() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: infernoTLineups.length,
      smoke: infernoTLineups.filter(item => item.type === "smoke").length,
      molly: infernoTLineups.filter(item => item.type === "molly").length,
      flash: infernoTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return infernoTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Inferno – T Utility</h1>
