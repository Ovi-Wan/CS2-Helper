...
import infernoCTLineups from "../../../data/inferno-ct-lineups";
...
export default function CtInferno() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: infernoCTLineups.length,
      smoke: infernoCTLineups.filter(item => item.type === "smoke").length,
      molly: infernoCTLineups.filter(item => item.type === "molly").length,
      flash: infernoCTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return infernoCTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Inferno – CT Utility</h1>
