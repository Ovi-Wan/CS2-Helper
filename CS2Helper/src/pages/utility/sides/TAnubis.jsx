...
import anubisTLineups from "../../../data/anubis-t-lineups";
...
export default function TAnubis() {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("all");

  const counts = useMemo(() => {
    return {
      all: anubisTLineups.length,
      smoke: anubisTLineups.filter(item => item.type === "smoke").length,
      molly: anubisTLineups.filter(item => item.type === "molly").length,
      flash: anubisTLineups.filter(item => item.type === "flash").length,
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    return anubisTLineups.filter((item) => {
      const matchesText =
  ...
  return (
    <div>
      <div className="title-search-row">
        <h1 className="page-title">Anubis – T Utility</h1>
